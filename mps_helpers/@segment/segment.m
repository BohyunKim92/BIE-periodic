...
classdef segment < handle & pointset
    ...
    methods
      function s = segment(M, p, qtype, o)
        ...
        if nargin<3, qtype='c'; end  %Clenshaw-Curtis          % default quadrature type
        if nargin<4, o=[]; end
      ...
      % See also: SEGMENT
        ...
        for s=segs
          ...
          s.t = (1+z)/2;               % t in [0,1], increasing, if N = 20, take s.t(1) = 1/40, s.t(2) = 1/40+1/20. so on 
          s.x = s.Z(s.t);
    ...
    methods(Static)    % these don't need segment obj to exist to call them...
      ...
      s = shiftedradial(M, fs, a1, varargin) 
